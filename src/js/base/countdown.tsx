...
import { useCallback, useDispatch, useEffect, useState } from "./react_base"
import { updateAuctionState } from "./actions"
...
function Countdown(props: CountdownProps) {
  ...
  const tick = useCallback(() => {
    ...
    const secondsRemaining = Math.max(0, Math.ceil(((endTime?.ts || 0) - DateTime.now().ts) / 1000))

    if (secondsRemaining === 0) {
      dispatch(updateAuctionState())
    }
